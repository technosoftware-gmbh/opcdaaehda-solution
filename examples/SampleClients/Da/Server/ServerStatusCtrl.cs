...
namespace SampleClients.Da.Server
{
    ...
    public class ServerStatusCtrl : System.Windows.Forms.StatusStrip
    {
		...
		/// </summary>
		public void Start(TsCDaServer server)
		{
			mServer_ = server;
		...
		private void UpdateTimer_Tick(object sender, System.EventArgs e)
		{
			try
			{
				GetServerStatusEventHandler callback = new GetServerStatusEventHandler(mServer_.GetServerStatus);
		...
		/// <summary>The asynchronous delegate for GetStatus.</summary>
		/// <remarks>
		/// If the client relies on asynchronous refresh and does not periodically access the server, this can be used to periodically get the status from the server to determine that the server is still connected and available. 
		/// </remarks>
		public delegate OpcServerStatus GetServerStatusEventHandler();
